...
public abstract class GameLoop implements Runnable {
    ...
    // Every write to game state happens on whichever thread runs this frame, and only one thread
    // runs a given loop at a time. update() runs GameActionSystem first, applying the input, bot
    // and ping actions other threads queued since the last frame, so no lock is taken below.
